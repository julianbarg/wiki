...
The declared intention of this literature review is to illuminate greenwashing as a broad literature. Specifically, we aim to include in the final corpus recent trends ranging from *woke washing* [@Sobande2019] and *symbolic civil rights* [@Edelman2016; @Buchter2021] to such extremes as *eco-criminality* [@Torelli2020]. To shed light on a diverse literature, we follow the model of @Oliveira2019 for a broad literature search: we will make a dedicated effort at the start of our search to identify additional keywords *before* we create the final corpus within relevant disciplines [see also @Marescaux2021]. For a step-by-step of our methodology, see [[Table 1|Greenwashing Literature Review#tables]]. This review builds on @Lyon2015 and hence focuses on articles published in 2014 and after.

We identify the relevant body of literature for our review in three phases. Toward the completion of phase one, we have already conducted a full-text search for the terms "greenwash" and "greenwashing" on EBSCO Business Source Complete. We will use the 1,265 identified articles to create an initial list of keywords that describe any kind of greenwashing. In phase two, we will identify the relevant corpus for our review. For that purpose, we search for our initial keywords in article titles, abstracts, and keywords across our list of target journals (see Table 3). The authors will then collectively screen the articles for relevance. In the third phase, we will conduct a quantitative analysis. A citation analysis ensures that our literature search has achieved saturation, and the creation of topic models will aid our subsequent review of the literature. Instead of including graphs, we provide a web app that users can utilize to trace the development of the literature in multiple ways.
...
<br/>       | *Step 3*: Initial keyword list                | Within the extracted paragraphs, we search for all terms that describe a form of greenwashing. These terms constitute our list of initial keywords.
...
</br>       | *Step 11*: Dashboard                          | Provide the final and all interim corpuses in an annotated shiny web app [@Chang2021a]. The web app can be used both to filter and search our identified articles, and to plot the evolution of the literature.
